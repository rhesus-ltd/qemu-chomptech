				       		...
				       		unsigned	int	size)
{
	...
	case R_NFC_CTRL:
		...
		//this should move to an actual command
			break;
	case R_ECC_CTRL:
...
static void 
chomp_nfc_write(void *opaque, hwaddr addr, uint64_t value64,
		unsigned int size)
{
	CHOMP_NFCState *s = opaque;
	...
	case R_NFC_COMMAND + 9:
		 ...
		 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n"
		 * ); if(s->cycle == 0) { s->cmd = reg_info; printf("Commmand
		 * Cycle\n"); s->addr_cnt = 0; } else { s->end = reg_info;
		 * printf("End Cycle\n"); printf("Got NAND command: %02x, ",
		 * s->cmd); for(int i = 0; i < s->addr_cnt ; i++) {
		 * printf("%02x, ",s->addr[i]); } printf("%02x\n", s->end); }
		 * }
		 */
		...
		if (value & NFC_CMD_REG_LAST_BIT) {
			...
			/*
			 * if(s->cmd == 0x0) { if(s->cmd == 0x0 && s->end ==
			 * 0x30) { printf("RESET STATUS READ!\n"); dest[0] =
			 * 0; dest[1] = 0; dest[2] = 0; dest[3] = (3 << 5); }
			 * 
			 * uint32_t column = s->addr[0] << 8 | s->addr[1];
			 * uint32_t row = (s->addr[2] << 16) | (s->addr[3] <<
			 * 8) | s->addr[4];
			 * 
			 * s->addr_int = column << 24 | row;
			 * 
			 ...
			 * 
			 * for(int i = 0; i < 0x800/4; i++) {
			 * address_space_stl_le(s->as, 0x0802f800 + i * 4,
			 * *((uint32_t*)dest + i), MEMTXATTRS_UNSPECIFIED,
			 * NULL); } } else
			 */ if (s->cmd == 0xFF) {
	...
	case R_NFC_CTRL:
		...
		if (value & NFC_CTRL_REG_CMD_VALID_BIT) {
			 ...
			 * *(unsigned int*)dest); for(int i = 0; i < 0x100/4;
			 * i++) { // This is wrong.. so wrong.. FIXME:
			 * printf("Store: %08x to %08x\n", *((uint32_t*)dest
			 * + i), 0x0802f800 + i * 4);
			 * address_space_stl_le(s->as, 0x0802f800 + i * 4,
			 * *((uint32_t*)dest + i), MEMTXATTRS_UNSPECIFIED,
			 * NULL); //copy the shit to the guest memory }
			 */
			//printf("Command valid bit set\n");
			r |= NFC_CTRL_REG_CMD_DONE_BIT;
			//We are fast..
				printf("COMMAND DONE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
		}
...
static void 
chomp_nfc_realize(DeviceState * dev, Error ** errp)
{
	...
	/*
	 * if (s->x != 1) { chomp_nfc_init_sram(sbd, &s->itf[itfn]); itfn++;
	 * }
	 * 
	 * if (s->x & 0x1) {  chomp_nfc_init_nand(sbd, &s->itf[itfn]); } else if
	 * (s->x == 4) { chomp_nfc_init_sram(sbd, &s->itf[itfn]); }
	 */
	...
	if (dinfo) {
		...
		/*
		 * The original U-Boot accesses the flash at 0xFE000000
		 * instead of 0xFF800000 (if there is 8 MB flash). So remap
		 * flash access if the image is smaller than 32 MB.
		 * 
		 * chomp_nfc_register(0x100000000ULL - MP_FLASH_SIZE_MAX,
		 * "chomptech.flash", flash_size, blk, 0x10000,
		 * MP_FLASH_SIZE_MAX / flash_size, 2, 0x00BF, 0x236D, 0x0000,
		 * 0x0000, 0x5555, 0x2AAA, 0);
		 */
	}
}

static void 
chomp_nfc_init(Object * obj)
{
...
static void 
chomp_nfc_class_init(ObjectClass * klass, void *data)
{
