	...
	static void	uart_update_irq(ChompUART * s)
{
	unsigned int	irq;
	/*
	 * if (s->rx_fifo_len) s->regs[R_STATUS] |= STATUS_IE;
	 */
...
static void 
uart_update_status(ChompUART * s)
{
	uint32_t	r;

	r = s->regs[R_UART_CFG2];
	//r &= ~7;
	r |= TX_FIFO_EMPTY | TX_END;
...
static void 
chomp_uart_reset(DeviceState * dev)
{
...
static void 
uart_rx(void *opaque, const uint8_t * buf, int size)
{
	ChompUART      *s = opaque;
...
static int 
uart_can_rx(void *opaque)
{
	//ChompUART * s = opaque;

	return 1;
	//s->rx_fifo_len < sizeof(s->rx_fifo);
}

static void 
uart_event(void *opaque, QEMUChrEvent event)
{

}

static void 
chomp_uart_realize(DeviceState * dev, Error ** errp)
{
...
static void 
chomp_uart_class_init(ObjectClass * klass, void *data)
{
...
static void 
chomp_uart_register_types(void)
{
